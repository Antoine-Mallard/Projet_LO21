#include "Calendar.h"
#include "timing.h"
//Surcharge d'opérateurs et affichage pour le namespace TIME
...
void Tache::setId(const QString& str){//on vérifie si la tache n'existe pas déjà
...
{
    if (this==&p) return *this;//on vérifie s'il ne s'agit pas du meme objet
    this->~Projet();//on détruit l'objet existant
    for(unsigned int i=0; i<p.nb; i++) addTache(new Tache(*p.taches[i]));//nb et nbMax sont mis à jour dans addTache
    return *this;
}
...
virtual const Date& TacheComposite::getEcheance()const//Borne supérieure du tableau A REVOIR
{
...
Tache& TacheComposite::ajouterTache(const QString& id, const QString& t, const Duree& dur, const QDate& dispo){//on ajoute une tache si elle n'existe pas deja
    ...
    Tache* newt=new Tache(id,t,dur,dispo);//si la Tache n'existe pas deja on la crée
...
{
    if (this==&um) return *this;//on vérifie s'il ne s'agit pas du meme objet
    this->~TacheManager();//on détruit l'objet existant
    for(unsigned int i=0; i<um.nb; i++) addItem(new Tache(*um.taches[i]));//nb et nbMax sont mis à jour dans addItem
    return *this;
}
...
TacheManager::~TacheManager(){//on sauvegarde dans un fichier à la destruction
    if (file!="") save(file);
    for(unsigned int i=0; i<nb; i++) delete taches[i];
    delete[] taches;
    file="";
...
Tache& TacheManager::ajouterTache(const QString& id, const QString& t, const Duree& dur, const QDate& dispo, const QDate& deadline, bool preempt){//on ajoute une tache si elle n'existe pas deja
    if (trouverTache(id)) throw CalendarException("erreur, TacheManager, tache deja existante");
    Tache* newt=new Tache(id,t,dur,dispo,deadline,preempt);//si la Tache n'existe pas deja on la crée
    addItem(newt);//on l'ajoute au tableau
	return *newt;
}
...
TacheManager& TacheManager::getInstance(){
    //si l'instance interne n'est pas encore créée, on doit le faire
...
void TacheManager::libererInstance(){
    //si l'instance interne n'est pas encore libérée, on doit le faire
